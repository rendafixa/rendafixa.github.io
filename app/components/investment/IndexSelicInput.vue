...
const isValid = computed(() => {
  if (!selic.value) return false
  return parseInt(selic.value.toString()) > 0
})

const errorMessage = computed(() => {
  if (!selic.value) return 'Obrigatório'
  if (parseInt(selic.value.toString()) <= 0) return 'Deve ser um número positivo'
  return ''
})
</script>
