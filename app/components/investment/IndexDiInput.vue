...
const isValid = computed(() => {
  if (!di.value) return false
  return parseInt(di.value.toString()) > 0
})

const errorMessage = computed(() => {
  if (!di.value) return 'Obrigatório'
  if (parseInt(di.value.toString()) <= 0) return 'Deve ser um número positivo'
  return ''
})
</script>
